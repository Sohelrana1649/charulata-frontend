...
export default function ProductDetailPage() {
  ...
  // Reviews hooks & state
  const { data: reviewsResponse, isLoading: reviewsLoading } = useGetProductReviewsQuery(product?._id, {
    skip: !product?._id,
  });
  ...
  const activePrice = isSale ? salePrice : price;
  ...
  const handleThumbNext = (e?: React.MouseEvent) => {
    e?.stopPropagation();
    if (thumbStartIndex + maxVisiblePhotos < galleryImages.length) {
      setThumbStartIndex(prev => prev + 1);
    }
  };

  const handleThumbPrev = (e?: React.MouseEvent) => {
    e?.stopPropagation();
    ...
    <div className="mx-auto max-w-[1536px] 2xl:max-w-[1680px] px-4 sm:px-6 lg:px-10 xl:px-12 w-full py-4 sm:py-8 flex-1 space-y-8 sm:space-y-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-12 items-start">
        ...
        <div className="lg:col-span-5 xl:col-span-5 space-y-3.5 lg:sticky lg:top-24 max-w-md mx-auto lg:max-w-none w-full">
          ...
          {/* Interactive Multi-Angle Thumbnails & Video Carousel (Hybrid Touch-Swipe & Desktop Hover Arrows UX) */}
          {(galleryImages.length > 1 || product.videoUrl) && (
            <div className="space-y-1.5">
              ...
              <div className="flex items-center justify-between gap-3 w-full py-1 relative group/thumbrow">
                {/* 1. Photo Thumbnails FIRST (Clean Touch-Swipe Container with Desktop Hover Arrows) */}
                <div className="relative flex items-center flex-1 min-w-0">
                  {/* Left Arrow on Hover for Desktop */}
                  ...
                  {/* Right Arrow on Hover for Desktop */}
                  {galleryImages.length > 3 && (
                    <button
                      type="button"
                ...
                {/* 2. Product Video Thumbnail SECOND (Live Auto-Playing Muted Video Preview) */}
                {product.videoUrl && (
                  <button
                    type="button"
                    onClick={() => {
                  ...
                  >
                    {/* Live Silent Looping Video Preview with Optimized Preload */}
                    <video
                      src={product.videoUrl}
                      autoPlay
                      loop
                    ...
                    {/* Minimalist VIDEO Badge on Corner */}
        ...
        {/* Right Details Column (lg:col-span-7 xl:col-span-7) */}
        <div className="lg:col-span-7 xl:col-span-7 space-y-5 sm:space-y-6">
          
          {/* Header Tag, Title, SKU & Rating */}
          ...
          <div className="bg-card border border-border p-4 rounded-2xl shadow-2xs space-y-2">
            ...
            {/* Individual Product Flash Sale Deal Timer Banner */}
          ...
          {/* Action CTA Buttons (Buy Now & Add to Cart) */}
              ...
              <a 
                ...
                onClick={() => {
                  if (process.env.NODE_ENV !== 'production') {
                    console.log('WhatsApp Order product payload:', product);
                  }
                }}
                target="_blank" 
          ...
          {/* Trust & Guarantee Grid Cards (3 Pillars) */}
